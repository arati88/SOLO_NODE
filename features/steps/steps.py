...
def step_missing_transaction_id(context):
    context.txn = {"amount": 500.0, "merchant_id": "M101"}
    context.exception = None
...
def step_missing_merchant_id(context):
    context.txn = {"transaction_id": "TXN1001", "amount": 500.0}
    context.exception = None
...
def step_invalid_amount(context, amount):
    context.txn = {"transaction_id": "TXN1001", "amount": float(amount), "merchant_id": "M101"}
    context.exception = None
...
def step_run_authentication(context, token):
    try:
        import hmac
        if not hmac.compare_digest(str(token), context.expected_token):
...
def step_run_authentication_empty(context):
    try:
        import hmac
        if not hmac.compare_digest("", context.expected_token):
            raise PermissionError("Invalid API token")
        context.auth_result = True
        context.exception = None
    except PermissionError as e:
